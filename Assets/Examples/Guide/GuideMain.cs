...
public class GuideMain : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var bagBtn = _mainView.GetChild("bagBtn");
        bagBtn.onClick.Add(() => { _guideLayer.RemoveFromParent(); });

        _mainView.GetChild("n2").onClick.Add(() =>
        {
            GRoot.inst.AddChild(
                _guideLayer); //!!Before using TransformRect(or GlobalToLocal), the object must be added first
            var rect = bagBtn.TransformRect(new Rect(0, 0, bagBtn.width, bagBtn.height), _guideLayer);
